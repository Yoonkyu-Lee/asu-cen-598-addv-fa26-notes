...
**2026-09-22. 같은 성격의 결정이 하나 더 내려졌다. Lab 제출 코드를 커밋한다.**
`code/lab{N}/` 아래에 원본 그대로 두고 Lab 기록 페이지가 링크한다.
위험을 먼저 알렸다. Pages 는 검색엔진에 색인되고 저장소도 Public 이라 **들어올 경로가 있다.**
아직 채점 중인 2인 1조 과제이므로 파트너의 코드도 같이 게시된다.
...
**같은 날 둘째 결정. Lab 코드에서 주석을 전부 뺐고, 코드는 페이지 안에 전부 싣는다.**
사용자가 원한 것은 파일 이름 목록이 아니라 **내용, 커밋 변경사항 수준의 설명**이었다.
그리고 Lab 코드에는 주석을 두지 않는다. 주석 없이 읽히게 하는 것이 기록 페이지의 일이다.
주석이 없으니 예전에 인라인을 막던 이유(한국어 주석이 영어 모드에 남는 것)도 사라졌다.
**코드는 `codeview.js` 뷰어로 싣는다.** 접히는 트리와 diff, 줄 단위 이중 언어 설명 카드.
데이터는 Lab 저장소 git 히스토리에서 `scripts/codeview_build.py` 가 굽는다. 절차는 `write-lab-note`.
`code/lab{N}/` 의 파일 사본은 그대로 두고 링크도 남긴다. 스크립트가 꺼진 독자를 위한 것이다.
...
| `code/lab{N}/` (Lab 제출 코드) | **O.** 2026-09-22 결정. 위 참조 |
| Lab 문제지 원문 | **X.** 우리 저작물이 아니다. 요구사항 요약으로 쓴다 |
| Quiz, Exam 문제 | X |
