...
**코드를 페이지 본문에 인라인하지 않고 파일로 두는 이유는 따로 있다.** 이중 언어 규칙이다.
`pre.code` 안의 한국어 주석은 영어 모드에서 그대로 남아 `langcheck.mjs` 가 실패시킨다.
전문을 그대로 박으려면 주석마다 영어 짝을 달아야 하는데 수백 줄이면 유지가 안 된다.
**파일은 링크하고 본문에는 이중 언어 주석을 단 발췌를 둔다.**
