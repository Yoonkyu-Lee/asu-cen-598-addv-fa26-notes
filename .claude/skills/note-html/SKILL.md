...
예외는 셋이다. PDF 리더(`reader.js`), **실습 입문 페이지의 시뮬레이션 뷰어(`sim.js`)**,
**애니메이션 슬라이더(`anim.js`)**. 셋 다 repo 안 로컬 파일이므로 "외부 의존성 없음" 은
유지된다. 셋 다 없어도 노트 본문은 그대로 읽혀야 한다. `vendor/pdf.js/`, `slides/*.pdf`,
`sim.js`, `anim.js` 가 없거나 로드에 실패해도 본문은 막히지 않는다.
