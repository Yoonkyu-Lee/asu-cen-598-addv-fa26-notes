...
예외는 넷이다. PDF 리더(`reader.js`), **실습 입문 페이지의 시뮬레이션 뷰어(`sim.js`)**,
**애니메이션 슬라이더(`anim.js`)**, **Lab 기록 페이지의 코드 뷰어(`codeview.js`)**.
넷 다 repo 안 로컬 파일이므로 "외부 의존성 없음" 은 유지된다. 넷 다 없어도 노트 본문은
그대로 읽혀야 한다. `vendor/pdf.js/`, `slides/*.pdf`, `sim.js`, `anim.js`, `codeview.js` 가
없거나 로드에 실패해도 본문은 막히지 않는다. 코드 뷰어 카드 안에는 저장소 파일로 가는
정적 안내 문단을 두고, 뷰어가 뜨면 그 자리를 덮어쓴다.
**리더도 뷰어도 슬라이더도 없어도 되는 부가 기능이다.** `anim.js` 가 없으면 첫 프레임이
그대로 보이고 조작 막대가 안 생긴다.
...
| `.play` | 인터랙티브 도구 | 흰 카드 |
| `figure.anim` | 애니메이션 슬라이더 (`anim.js`) | 흰 카드 + 조작 막대 |
| `.cv[data-cv]` | 코드 뷰어 (`codeview.js`). 트리 + diff/원문 + 줄 단위 설명 카드 | 흰 카드, 왼쪽 트리 |
| `button.cv-go` | 본문에서 뷰어의 특정 줄로 데려가는 바로가기 | 작은 mono 버튼 |
...
**이 과목 덱은 같은 그림을 여러 장에 걸쳐 움직이는 구간이 많다.** PDF 로 변환되면 그게
그냥 비슷한 그림 여러 장이 되고, 노트로 옮길 때는 대개 표나 "세 컷 요약" 으로 납작해진다.
**납작하게 만들지 말고 슬라이더로 되살린다.** 독자가 앞뒤로 밟아보게 하는 것이 목적이다.

**애니메이션 구간을 찾는 법.** `node scripts/pdftext.mjs slides/<stem>.pdf` 로 쪽별 텍스트를
뽑고, **제목이 같은 연속 쪽**을 본다. 텍스트가 거의 같은데 값만 바뀌면 애니메이션이다.
**렌더해서 눈으로 확인한다.** 텍스트가 같아도 그냥 불릿이 하나씩 나타나는 것뿐이면
슬라이더로 만들 값어치가 없다. 그림이 실제로 움직여야 한다.

| 만든다 | 안 만든다 |
|---|---|
| 값·포인터·내용물이 프레임마다 바뀌는 그림 | 불릿만 하나씩 나타나는 쪽 |
| 부품을 하나씩 붙여가는 구성도 | 서로 다른 문제를 다루는 이웃 쪽 |
| 같은 캔버스에 주석만 갈아끼우는 타이밍 도해 | 코드 리스팅이 바뀌는 쪽 |

**나란히 비교가 요점이면 슬라이더로 만들지 않는다.** 컷이 서넛뿐이고 한 화면에 다 들어가면
**정적 figure 가 이긴다.** 슬라이더는 한 번에 한 컷만 보여주므로 비교를 오히려 방해한다.
실제 판정 사례 셋을 적어둔다.

| 구간 | 판정 | 이유 |
|---|---|---|
| L01 슬라이드 13&ndash;16 (Design Flow 층 쌓기) | **안 만든다** | 네 장이 서로 다른 주제라 노트가 이미 섹션 넷으로 갈라 각각 도해를 뒀다 |
| L04 슬라이드 36&ndash;38 (reset ordering) | **안 만든다** | 세 컷이 세 가지 경우라, 나란히 놓은 기존 3 패널이 더 낫다 |
| L02 슬라이드 47&ndash;51 (testbench 쌓기) | **만든다** | 쌓는 순서 자체가 설명이고, 각 칸이 없으면 뭘 못 하는지가 순서에서 나온다 |

**다르게 판정했으면 여기 한 줄 보탠다.** 다음에 같은 구간을 다시 고민하지 않게.

**마크업.** 프레임은 HTML 에 선언하고 스크립트는 보이기만 갈아끼운다.

```html
<figure class="anim">
  <svg viewBox="0 0 700 250" role="img" aria-label="…">
    <g>…프레임마다 그대로인 뼈대. data-f 를 안 붙인다…</g>
    <text data-f="1 2 3 4" …>D0</text>
    <rect data-f="4" …/>
  </svg>
  ...
  …프레임 수만큼…
  <figcaption lang="ko">…</figcaption>
  <figcaption lang="en">…</figcaption>
</figure>
```

- **`data-f` 는 그 요소가 보이는 프레임 번호를 공백으로 나열한 것이다.** 범위 표기(`0-3`)를
  쓰지 않는다. CSS 의 `[data-f~="0"]` 가 스크립트 없이도 첫 프레임을 띄우려면 이 형태여야 한다.
- **프레임 개수는 `.animcap` 의 개수로 정해진다.** 그림에만 있고 설명이 없는 프레임은 못 만든다.
  이건 의도한 제약이다. 밟을 때마다 무슨 일이 일어났는지 한 줄은 읽혀야 한다.
- **`data-src` 는 그 프레임이 온 슬라이드 쪽수다.** 라벨은 CSS 가 만든다. 손으로 적지 않는다.
  **`data-slide` 와 다르다.** `data-src` 는 리더 앵커가 아니고 동기화에 관여하지 않는다.
- **`data-f` 는 되도록 잎 요소에 직접 붙인다.** 프레임마다 갈아끼우는 `<text>` 를 따로 `<g>` 로
  감싸면 그 글자가 다른 그룹의 도형 위에 있는 것으로 잡혀서 검증 경고가 쏟아진다.
- **같은 자리에 두 벌을 겹쳐 그리지 않는다.** 예를 들어 회색 라벨 위에 강조용 파란 라벨을
  덮어 그리면 그 프레임에서 **글자끼리 겹침으로 잡혀 검증이 실패한다.** 꺼진 프레임 목록과
  켜진 프레임 목록으로 **갈아끼운다.**
- 설명 문장은 전부 `.animcap` 과 `figcaption` 에 둔다. **SVG 안에 한국어 서술을 넣지 않는다.**
  포트 이름 같은 영어 라벨만 SVG 에 남긴다. 그러면 SVG 텍스트를 두 벌 만들 일이 없다.
- 조작 막대(`.animbar`)는 **스크립트가 직접 만든다.** HTML 에 적지 않는다. 그래야
  `anim.js` 가 없을 때 눌러도 아무 일 없는 버튼이 남지 않는다.

**프레임이 많으면 손으로 쓰지 말고 생성한다.** 프레임 9 개짜리는 `data-f` 목록이 수십 개라
손으로 맞추면 반드시 틀린다. 값 표(프레임별 상태)를 먼저 적고 짧은 스크립트로 SVG 를 찍어낸다.
**생성기는 저장소에 남기지 않는다.** 스크래치패드에서 돌리고 결과 마크업만 노트에 넣는다.

**검증.** `verify.mjs` 는 **첫 프레임만 본다.** 나머지 프레임은 이걸로 따로 돌린다.

```bash
node scripts/animshot.mjs notes/L{NN}-{topic}.html              # 전 프레임, exit 0 이어야 통과
node scripts/animshot.mjs notes/L{NN}-{topic}.html 4 --en --dark
node scripts/animshot.mjs notes/L{NN}-{topic}.html 4 --w=390
```

프레임마다 글자 겹침과 viewBox 이탈을 다시 재고 `shots/<stem>-anim01-fNN.png` 를 남긴다.
**전부 눈으로 연다.** 통과해도 건너뛰지 않는다.

**`.anim` 과 `.play` 는 다른 물건이다.** `.anim` 은 **슬라이드가 실제로 보여준 것을 되살리는**
것이고 순서가 정해져 있다. `.play` 는 **슬라이드에 없는 상황을 독자가 만들어보는** 것이다.
L05 가 둘 다 갖고 있다. 07 절의 `.anim` 은 슬라이드 23&ndash;31 쪽 그대로이고,
17 절의 `.play` 는 "가득 찼을 때 한 번 더 쓰면" 처럼 **슬라이드가 안 보여준 것**을 다룬다.
**둘을 합치려 하지 않는다.**
...
anim.js                                           애니메이션 슬라이더 (.anim 을 쓰는 노트가 공유)
codeview.js                                       코드 뷰어 (Lab 기록 페이지가 공유)
...
| 스크립트 | 하는 일 |
|---|---|
| `animshot.mjs` | `.anim` 을 프레임마다 찍고 겹침·이탈을 다시 잰다. `verify.mjs` 가 못 보는 구간 |
...
node scripts/animshot.mjs <노트 파일>      # .anim 이 있으면
```
