...
**같은 파일명 아래 장르가 둘이다.** 아래는 **공략** 쪽이고, Lab 을 끝낸 뒤에 남기는
**기록** 쪽은 `write-lab-note` 스킬이 단일 출처다. 기록 쪽은 절 구조도 분량도 다르니
**이 절을 따라 쓰지 말고 그쪽을 읽는다.**

공략은 과제를 펴놓고 앉았을 때 "이거 풀려면 뭘 봐야 하지"에 답하는 곳.
...
`verify.mjs` 는 `HW*.html` 과 `LAB*.html` 을 둘 다 본다. **이미 고쳐져 있다.**
HW 는 문제 번호를 `.sec-num` 에서, Lab 은 Task 번호를 `data-task` 에서 읽는다.
Task 로 안 나뉘는 Lab 은 `data-task` 를 안 쓰면 검사가 조용히 통과한다.
...
**도해에는 "그림 읽기" 를 붙이고, 그림에 나온 값을 하나도 건너뛰지 않고 풀어 쓴다.** 2026-09-22
사용자가 "이런 설명을 절대 생략하면 안 된다" 고 못박았다. 계기는 LAB1 의 그림 하나였다. 그림에
`captured 1011 = 11` 과 `gray 0100 → 1100, only 7 or 8 possible` 만 적혀 있었고, 왜 1011 이
잡히는지, 1100 이 왜 12 가 아니라 8 인지가 어디에도 없었다. 그림은 결론을 보여줄 뿐이다.

- 그림에 **숫자 · 비트열 · 신호 이름**이 나오면 본문에서 **그것이 어디서 왔는지 한 단계씩** 적는다.
  비트열이면 비트마다 (옛 값 / 새 값 / 그대로) 표로
- **같은 모양이 다른 뜻일 수 있으면 반드시 짚는다.** gray `1100` 은 binary 로 읽으면 12 이지만 8 이다
- 계산이 들어가면 식과 대입을 적는다 (`1000` XOR `0100` = `1100`). 대조표가 도움이 되면 넣는다
- 약칭 이름(`rq2_wptr`)은 글자 단위로 풀어준다
- 캡션은 요약이다. **풀이는 캡션이 아니라 그림 바로 뒤의 `.def` "그림 읽기" 블록**에 둔다
- 독자가 "이게 무슨 말이냐" 고 물을 만한 곳이 남았으면 아직 끝난 게 아니다
